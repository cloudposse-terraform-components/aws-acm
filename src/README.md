...
| <a name="requirement_aws"></a> [aws](#requirement\_aws) | >= 6.4.0, < 7.0.0 |

## Providers

| Name | Version |
|------|---------|
| <a name="provider_aws"></a> [aws](#provider\_aws) | >= 6.4.0, < 7.0.0 |
...
| <a name="input_ssm_parameter_name"></a> [ssm\_parameter\_name](#input\_ssm\_parameter\_name) | Name of the SSM parameter that stores the certificate ARN.<br/>Defaults to `""`, which keeps the computed `/acm/<domain_name>` name. Set this when a single<br/>account holds more than one certificate for the same `domain_name`, since the computed names<br/>would otherwise collide. | `string` | `""` | no |
